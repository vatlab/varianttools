...
def VariantReader(proj, table, export_by_fields, order_by_fields, var_fields,
                  geno_fields, export_alt_build, IDs, jobs):
    if jobs == 0 and len(IDs) < MAX_COLUMN and proj.store == "sqlite":
        # using a single thread
        return EmbeddedVariantReader(proj, table, export_by_fields,
                                     order_by_fields, var_fields, geno_fields,
        ...
        return StandaloneVariantReader(proj, table, export_by_fields,
                                       order_by_fields, var_fields, geno_fields,
                                       export_alt_build, IDs)
    else:
        # using multiple process to handle more than 1500 samples
        if len(IDs) // MAX_COLUMN + 2 > jobs:
            env.logger.info('Using {} processes to handle {} samples'.format(
                len(IDs) // MAX_COLUMN + 2, len(IDs)))
        return MultiVariantReader(proj, table, export_by_fields,
                                  order_by_fields, var_fields, geno_fields,
...
class BaseVariantReader:
    ...
    def __init__(self, proj, table, export_by_fields, order_by_fields,
                 var_fields, geno_fields, export_alt_build, IDs):
        self.proj = proj
        self.table = table
    ...
    def getQuery(self):
        ...
        if self.order_by_fields:
            order_fields, order_field_names = consolidateFieldName(
                self.proj, self.table, self.order_by_fields)
        ...
        tmp_fields = list(
            set(select_fields +
                (order_field_names if self.order_by_fields else [])))
        ...
        if self.geno_fields:
            for id in self.IDs:
                header = [
                    x.lower() for x in self.proj.db.getHeaders(
                        '{}_genotype.genotype_{}'.format(self.proj.name, id))
                ]
                for fld in self.geno_fields:
                    if fld.lower() in header:
                        select_clause += ', {}_genotype.genotype_{}.{}'.format(
                            self.proj.name, id, fld)
                    else:
                        select_clause += ', NULL'
        ...
        return 'SELECT {} {} {};'.format(select_clause, from_clause,
                                         order_clause)
...
#            from_clause, where_clause, order_clause)
    ...
    def getVariantQuery(self):
        ...
        select_clause, select_fields = consolidateFieldName(
            self.proj, self.table,
            ','.join(['variant_id', 'variant.ref', 'variant.alt'] +
                     self.var_fields), self.export_alt_build)
        # FROM clause
        from_clause = 'FROM {} '.format(self.table)
        fields_info = sum(
            [self.proj.linkFieldToTable(x, self.table) for x in select_fields],
        ...
        if self.order_by_fields:
            order_fields, order_field_names = consolidateFieldName(
                self.proj, self.table, self.order_by_fields + ',variant_id')
            order_clause = ' ORDER BY {}'.format(order_fields)
        else:
    ...
    def getSampleQuery(self, IDs):
        ...
        if self.table != 'variant':
            from_clause += ' LEFT OUTER JOIN variant on {0}.variant_id = variant.variant_id '.format(
                self.table)
        for id in IDs:
        ...
        # GROUP BY clause
        if self.order_by_fields:
            order_fields, tmp = consolidateFieldName(
                self.proj, self.table, self.order_by_fields + ', variant_id')
            order_clause = ' ORDER BY {}'.format(order_fields)
        else:
            order_clause = ' ORDER BY {}.variant_id'.format(self.table)
...
class EmbeddedVariantReader(BaseVariantReader):
    ...
    def __init__(self, proj, table, export_by_fields, order_by_fields,
        ...
        self.proj = proj
        self.var_fields = var_fields
        BaseVariantReader.__init__(self, proj, table, export_by_fields,
                                   order_by_fields, var_fields, geno_fields,
                                   export_alt_build, IDs)
...
class StandaloneVariantReader(BaseVariantReader):
    ...
    def __init__(self, proj, table, export_by_fields, order_by_fields,
        ...
        self.proj = proj
        ID_needed_idx = [
            id for id in IDs if not self.proj.db.hasIndex(
                '{0}_genotype.genotype_{1}_index'.format(self.proj.name, id))
        ]
        if len(ID_needed_idx) > 0:
            prog = ProgressBar('Creating indexes', len(ID_needed_idx))
            cur = self.proj.db.cursor()
            for idx, id in enumerate(ID_needed_idx):
                if not self.proj.db.hasIndex(
        ...
        self.worker = VariantWorker(proj.name, proj.annoDB, self.getQuery(), w,
                                    None)
        self.worker.start()
...
class MultiVariantReader(BaseVariantReader):
    ...
    def __init__(self, proj, table, export_by_fields, order_by_fields,
                 var_fields, geno_fields, export_alt_build, IDs, jobs,
                 transformToHDF5):
        BaseVariantReader.__init__(self, proj, table, export_by_fields,
                                   order_by_fields, var_fields, geno_fields,
                                   export_alt_build, IDs)
        ...
        p = VariantWorker(proj.name, proj.annoDB, self.getVariantQuery(), w,
                          lock)
        self.workers = [p]
        self.readers = [r]
        IDs = list(IDs)
        ...
        if self.proj.store == "sqlite" or transformToHDF5:
            ...
            with delayedAction(env.logger.info, 'Checking indexes'):
                ID_needed_idx = [
                    id for id in IDs if
                    not self.proj.db.hasIndex('{0}_genotype.genotype_{1}_index'
                                              .format(self.proj.name, id))
                ]
            if len(ID_needed_idx) > 0:
                prog = ProgressBar('Creating indexes', len(ID_needed_idx))
            ...
            for i in range(self.jobs - 1):
                ...
                p = VariantWorker(proj.name, proj.annoDB,
                                  self.getSampleQuery(subIDs), w, lock)
            ...
            for HDFfileName in samplefiles:
                ...
                if filename in sampleFileMap:
                    ...
                    if len(overlapSamples) > 0:
                        ...
                        self.jobs += 1
                        if multiVCF == 0:
                            p = VariantWorker_HDF5(HDFfileName, overlapSamples,
                                                   self.geno_fields, w)
                        else:
                            p = VariantWorker_HDF5_multi(
                                proj.name, proj.annoDB, self.getVariantQuery(),
                                HDFfileName, overlapSamples, self.geno_fields,
                                lock, w)
                        self.workers.append(p)
                        self.readers.append(r)
...
class VariantWorker_HDF5_multi(Process):
    ...
    def __init__(self, dbname, annoDB, query, HDFfileName, samples, geno_fields,
                 lock, output):
        self.dbname = dbname
        self.annoDB = annoDB
        self.query = query
    ...
    def run(self):
        ...
        genoinfo_fields = [
            field.replace("_geno", "") for field in self.geno_fields
        ]
        ...
        for rec in cur:
            if rec[0] != last_id:
                ...
                sub_all = accessEngine.get_geno_by_row_pos(
                    rec[0], chr, sortedID, self.samples, genoinfo_fields)
                ...
                if len(self.geno_fields) == 0:
                    ...
                    val = np.zeros(
                        shape=(numcol * (len(self.geno_fields))), dtype=float)
                    ...
                    if len(genoinfo_fields) > 0:
                        for pos, field in enumerate(genoinfo_fields):
                            ...
                            for col in range(numcol):
                                val[col * len(self.geno_fields) + pos +
                                    1] = genoInfo[col]
...
class VariantWorker_HDF5(Process):
    ...
    def run(self):
        ...
        if "GT" in genoinfo_fields:
            genoinfo_fields.remove("GT")
        for rownames, colnames, sub_all in accessEngine.get_all_genotype_genoinfo(
                self.samples, [], genoinfo_fields):
            ...
            if len(self.geno_fields) == 0:
                ...
                info = np.zeros(
                    shape=(numrow, numcol * (len(self.geno_fields) + 1)),
                    dtype=float)
                for col in range(numcol):
                    info[:, col] = genoType[:, col]
                if len(genoinfo_fields) > 0:
                    for pos, field in enumerate(genoinfo_fields):
                        ...
                        for col in range(numcol):
                            info[:, col * len(self.geno_fields) + pos +
                                 1] = genoInfo[:, col]
...
class VariantWorker(Process):
    ...
    def run(self):
        ...
        if '{}_genotype.'.format(self.dbname) in self.query:
            db.attach(
                '{}_genotype.DB'.format(self.dbname),
                '{}_genotype'.format(self.dbname),
                lock=self.lock)
        for anno in self.annoDB:
            db.attach(os.path.join(anno.dir, anno.filename), lock=self.lock)
        cur = db.cursor()
