...
class Project:
    ...
    def close(self):
        ...
        try:
            ...
            sys.stderr.write('Failed to remove temporary directory {0}: {1}\n'.format(repr(runOptions.temp_dir), e.strerror))
    ...
    def mergeSamples(self):
        ...
        for idx, (name, ids) in enumerate(merged.iteritems()):
            # merge genotypes
            prog = ProgressBar('Merging {} ({}/{})'.format(name, idx + 1, len(merged)), len(ids) * 2 + 2)
            ...
            cur.execute('SELECT sql FROM {}_genotype.sqlite_master WHERE type="table" AND name="genotype_{}"'.format(self.name,
                ids[0]))
            #
            # sometimes the sql has CREATE TABLE "genotype_1" instead of CREATE TABLE genotype_1 
            # when the sql is automatically generated by ALTER TABLE RENAME TO...
            #
            # this command first try to replace the whole "genotype_1" thing, then genotype_1..
            sql = cur.fetchone()[0].replace('"genotype_{}"'.format(ids[0]), '{}_genotype.__tmp_{}'.format(self.name, ids[0]))
            sql = sql.replace('genotype_{}'.format(ids[0]), '{}_genotype.__tmp_{}'.format(self.name, ids[0]))
