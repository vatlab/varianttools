...
class Project:
    ...
    def mergeSamples(self):
        ...
        for name, ids in merged.iteritems():
            ...
            sql = cur.fetchone()[0].replace('"genotype_{}"'.format(ids[0]), new_table)
            sql = sql.replace('genotype_{}'.format(ids[0]), new_table)
            ...
            for id in ids:
                ...
                try:
                    cur.execute('INSERT INTO {1} SELECT * FROM {0}_genotype.genotype_{2};'.format(
                        self.name, new_table, id))
                except Exception as e:
                    try:
                        # remove existing temporary table if exists
                        self.db.removeTable(new_table)
