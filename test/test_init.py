...
class TestInit(ProcessTestCase):
    ...
    def testInitFromParentalProject(self):
        ...
        self.runCmd(
            '''vtools select variant --samples "sample_name like 'NA12%'" -t na12'''
        )
        self.assertProj(
            numOfVariants={
                'variant': 288,
        ...
        self.assertSucc(
            'vtools init test --parent parent --variants na12 --store ' +
            self.storeMode)
        self.assertProj(
            numOfVariants={
                'variant': 280,
        ...
        self.assertFail(
            'vtools init test --parent parent --variants na12 --genotypes GD>10'
        )
        # create project with only homozygous genotype
        self.assertSucc(
            'vtools init test --parent parent --variants na12 --genotypes GT=1 -f --store '
            + self.storeMode)
        self.assertProj(
            numOfVariants={
                'variant': 280,
        ...
        self.assertSucc(
            '''vtools init test --parent parent --samples "sample_name like 'NA1%'" -f  --store '''
            + self.storeMode)
    ...
    def testGenotypes_sample(self):
        ...
        self.runCmd('vtools import vcf/CEU.vcf.gz --build hg18')
        self.runCmd(
            'vtools import --format fmt/genotypes txt/genotypes.txt --build hg18'
        )
        shutil.move('test.proj', 'parent/test.proj')
        shutil.move('test_genotype.DB', 'parent/test_genotype.DB')
        self.assertSucc(
            'vtools init test --parent parent --variants variant --samples "filename like \'%geno%\'" --genotypes GT=1 --store '
            + self.storeMode)
    ...
    def testChildren(self):
        ...
        self.assertSucc('vtools init test --children ceu sam1 --store ' +
                        self.storeMode)
        self.assertProj(
            numOfVariants={
                'variant': 577,
