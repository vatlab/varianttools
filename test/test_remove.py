...
class TestRemove(ProcessTestCase):
    ...
    def setUp(self):
        ...
        if self.storeMode == "sqlite" and os.path.isfile(
                'TestRemove_sqlite.tar.gz'):
            self.runCmd('vtools admin --load_snapshot TestRemove_sqlite.tar.gz')
        elif self.storeMode == "hdf5" and os.path.isfile(
                "TestRemove_hdf5.tar.gz"):
            ...
            self.runCmd(
                'vtools import vcf/input_nogeno.vcf --build hg18 --sample_name input.tsv'
            )
            self.runCmd('vtools phenotype --from_file phenotype/phenotype.txt')
            self.runCmd('vtools use ann/testNSFP.ann')
            self.runCmd(
                'vtools select variant --samples "filename like \'%CEU%\'" -t CEU'
            )
            self.runCmd(
                'vtools select variant --samples "aff=\'1\'" -t unaffected')
            self.runCmd(
                'vtools update CEU --samples "filename like \'%CEU%\' and aff=\'2\'" --from_stat "CEU_cases_num=#(alt)"'
            )
            self.runCmd(
                'vtools import vcf/SAMP2.vcf --geno_info DP_geno --var_info DP --build hg18'
            )
            if self.storeMode == "sqlite":
                self.runCmd(
                    'vtools admin --save_snapshot TestRemove_sqlite.tar.gz "Snapshot for testing command remove"'
                )
            elif self.storeMode == "hdf5":
                self.runCmd(
                    'vtools admin --save_snapshot TestRemove_hdf5.tar.gz "Snapshot for testing command remove"'
                )
    ...
    def testRemoveFields(self):
        ...
        self.assertOutput(
            ...
            'output/remove_field_before.txt',
            partial=-3)
        self.runCmd('vtools remove fields CEU_cases_num gene_name DP')
        self.assertOutput(
            'vtools show table variant',
            'output/remove_field_after.txt',
            partial=-3)
    ...
    def testRemovePhenotype(self):
        ...
        if self.storeMode == "hdf5":
            self.assertOutput('vtools show samples',
                              'output/remove_phenotype_output.txt')
        elif self.storeMode == "sqlite":
            self.assertOutput('vtools show samples',
                              'output/remove_phenotype_sqlite.txt')

    def testRemoveGenoField(self):
        # runCmd('vtools import vcf/SAMP2.vcf --geno_info DP_geno --var_info DP--build hg18')
        self.maxDiff = None
        if self.storeMode == "sqlite":
            self.assertOutput('vtools show genotypes',
                              'output/remove_genofield_before_sqlite.txt')
        elif self.storeMode == "hdf5":
            self.assertOutput('vtools show genotypes',
                              'output/remove_genofield_before_hdf5.txt')
        ...
        if self.storeMode == "sqlite":
            ...
            self.assertOutput('vtools show genotypes',
                              'output/remove_genofield_after_sqlite.txt')
        elif self.storeMode == "hdf5":
            self.assertSucc('vtools remove geno_fields DP')
            self.assertOutput('vtools show genotypes',
                              'output/remove_genofield_after_hdf5.txt')
